...
static uint8_t makeConstant(Value value){
    ...
    if(constant > UINT8_MAX){
        error("Too many constants in single chunk.");
        return 0;
    }
...
static void patchJump(int offset){
    ...
    if(jump > UINT16_MAX){
        error("Requested jump exceeds max jump length");
    }
...
static void addLocal(Token name){
    if(current->localCount == UINT8_COUNT){
        error("Too many local variables declared in scope.");
        return;
    }
...
static void grouping(bool canAssign){
    expression();
    consume(TOKEN_RIGHT_PAREN, "Exprected closeing ) after expression.");
}
...
static void super_(bool canAssign){
    if (currentClass == NULL){
        ...
        error("Cannot use 'super' in class without a superclass");
    }
    ...
    if(!check(TOKEN_RIGHT_PAREN)){
        do{
            ...
            if(current->function->arity > 255){
                errorAtCurrent("Can't have more than 255 parameters per function.");
            }
